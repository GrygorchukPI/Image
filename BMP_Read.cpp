...
void BMP::ReadFile(Image &image, const std::string &source)
{
...
void BMP::FileToBuffer(const std::string &source)
{
    FILE* fileBMP;

    fileBMP = fopen(source.c_str() , "rb" );
    if(fileBMP == nullptr){printf("Cannot open file.\n"); exit(1);}
...
void BMP::BufferToBitmap()
{
    ///EXTRA BITS !!!!!

    this->ReadHeaderFromBuffer();
...
void BMP::ReadHeaderFromBuffer()
{
    ...
    this->bit_Count = this->info_header.BitCount;
