...
void BMP::WriteFile(Image &image, const std::string &source)
{
    this->GetBitmap(image);
    this->BitmapToBuffer();
    this->BufferToFile(source);
}

void BMP::BitmapToBuffer()
{
    /////!!!!!!!!!!!!!! EXTRA BITS!!!!!!!!!!!!
    this->extra_Bits = 0;
...
void BMP::BufferToFile(const std::string &source)
{
    //Write file
    FILE * newBMPfile;
    newBMPfile = fopen (source.c_str(), "wb");
    if(newBMPfile == nullptr){printf("Cannot create file.\n"); exit(1);}
