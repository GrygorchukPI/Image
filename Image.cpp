...
Image::Image(){}
Image::Image(uint32_t w, uint32_t h, RGB color)
{
    this->width  = w;
    this->height = h;
...
void Image::SetBitmapSize(uint32_t in_bitmapSize)
{
    this->bitmapSize = in_bitmapSize;
}

void Image::SetBitmap(uint8_t* in_bitmap)
{
    this->bitmap = new uint8_t[this->bitmapSize];
...
uint32_t Image::GetBitmapSize()
{
    return  this->bitmapSize;
}

uint8_t* Image::GetBitmap()
{
    return this->bitmap;
}
...
RGB  Image::GetPixel(uint32_t x, uint32_t y)
{
    RGB res;
...
void Image::FillCanvasByColor(RGB color)
{
...
///// Width Height
void Image::SetWidth(uint32_t in_w)
{
    this->width = in_w;
}

void Image::SetHeight(uint32_t in_h)
{
    this->height = in_h;
}
...
void Image::SwapPixel(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2)
{
    uint8_t swapR = 0;
    uint8_t swapG = 0;
    uint8_t swapB = 0;
...
void Image::SwapRows(uint32_t i, uint32_t j)
{
